...
% regularizes the linear classifier by encouraging W to be of small magnitude
lambda = 0.00001; % best result
num_train = size(train_image_feats, 1);
% preallocate
Ws = zeros(num_categories, size(test_image_feats, 2));
...
for i = 1:num_categories
    ...
    % trains linear svms based on training examples
    [W, B] = vl_svmtrain(train_image_feats', labels, lambda);
    % matrix of all categories' weight vectors
    Ws(i,:) = W';
    % matrix of all categories' offset vectors
    Bs(i) = B;
end
%construct a score table
Bs = repmat(Bs, 1, size(test_image_feats, 1));
score = Ws*test_image_feats' + Bs;
[~, I] = max(score);
