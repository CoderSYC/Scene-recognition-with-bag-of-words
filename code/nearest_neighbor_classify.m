...
% k nearest neighbors
k = 20;
% distances between train and test features
dist = vl_alldist2(train_image_feats', test_image_feats');
% how many kinds of labels
labels = unique(train_labels);
num_labels = size(labels, 1);
% sort the distances (small->large)
[~, sort_index] = sort(dist);
% initialize
N = size(test_image_feats,1);
poll = zeros(num_labels, N);
for i = 1:N
    % get top k nominees
    top_k = train_labels(sort_index(1:k, i));
    for j = 1:num_labels
        % how many votes among all labels
        votes = strcmp(labels(j), top_k);
        poll(j, i) = sum(votes);
    end
end
% choose the highest votes & get the prediction
[~, label_index] = max(poll,[],1);
predicted_categories = labels(label_index);
end
