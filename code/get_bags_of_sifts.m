...
for i = 1:N
    ...
    % get SIFT_features
    [~, features] = vl_dsift(img, 'Step', 3, 'Fast');
    % computes the nearest column of vocab to each column of features in Euclidean distance
    [index , ~] = vl_kdtreequery(kdtree , vocab' , single(features));
    % the number of elements in each bin
    % represent our training and testing images as bag of feature histograms
    count = hist(double(index), vocab_size);
    % normalize
    count = count ./ sum(count);
    count = count ./ norm(count);
    image_feats(i, :) = count;
end
