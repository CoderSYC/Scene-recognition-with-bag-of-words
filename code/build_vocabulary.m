...
sample = 10;
% preallocate
descriptors = zeros(128, N * sample);
for i = 1:N
    % convert to single precision
    img = im2single(imread(image_paths{i}));
    % get SIFT descriptors sampled at a regular step size from an image
    [~, features] = vl_dsift(img, 'Fast', 'Step', 10, 'Size', 8);
    % SIFT features from many training images
    descriptors(:, sample * (i-1) + 1 : sample * i) = features(:, 1:sample);
end
% clustering SIFT descriptors into k clusters.
[centers, ~] = vl_kmeans(descriptors, vocab_size);
% convert to single precision
vocab = single(centers');
fprintf('visual word vocabulary from training images is done!');
